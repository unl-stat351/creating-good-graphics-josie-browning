...
-   [x] Student's name is included at top of assignment
-   [x] Document compiles on a different machine
-   [x] Compiled document is formatted well
-   [x] All code in the document is contained in appropriately formatted code chunks
-   [x] Compiled document does not include long sections of printed data, making use of `head` and `tail` commands to show only a few rows of data during data cleaning.
-   [x] Student answers are below the `---` separator and above the next prompt, or in cases where questions are enumerated, student answers are indicated using the quote indicator, `>` and the placeholder text `Your answer here` has been removed and replaced with the answer.

### Task specific Criteria

1.  Graphical Critiques

-   [x] Each chart has an appropriate caption

-   [x] Each chart has appropriate alt-text that provides essential information to a visually impaired individual or screen-reader user.

-   [x] YAML chunk options are formatted correctly and compile.

-   [ ] Grammar of graphics table is filled out correctly.

-   [x] Analysis of effective communication in each chart is reasonable.

    -   [x] Charts that ineffectively show changes in vaccine rates over time across different age groups
        -   [x] are identified and
        -   [x] an explanation of *why* they are ineffective is provided.
    -   [x] Charts that are appropriate but perhaps suboptimal
        -   [x] have issues identified and
        -   [x] what would be changed to address the issue is provided and appropriate.
...
-   [x] Message Evaluation

    -   [x] Important messages from the data set are identified
    -   [x] Comparisons that should be easy to perform are identified
    -   [x] Obstacles are provided (if there are any, if not, "no obstacles" is an ok answer... but be careful with its use.)
